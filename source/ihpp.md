...
DL | string(16) | This value is used to specify the styling and functionality IPP should display to your customer. Please see section Payment Page Functionality for full details on DL value. Dl values will be confirmed by IPP upon account setup.
...
Once the session is established and working, your backend application notifies your website which then POSTs (GET also supported) the Session ID and Secure Session Token to IPP’s server URL.
...
SessionKey | string(64)	| The session key submitted by you only to IPP in the session initiation request.
...
Amount | string(10)	| Transaction amount in cent value e.g. $55.00 = 5500. This is the originally submitted amount and does not include any surcharge amount that may have been applied by IPP.
